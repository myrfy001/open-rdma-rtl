module bluerdma_top(
	...
	input  wire                 qsfpdd0_refclk_fht   ,
    input  wire                 qsfpdd0_refclk_fgt   ,
    input  wire     [3:0]       qsfpdd0_rx_p        ,
    input  wire     [3:0]       qsfpdd0_rx_n        ,
    output wire     [3:0]       qsfpdd0_tx_p        ,
    output wire     [3:0]       qsfpdd0_tx_n        ,

	inout  wire                 QSFP112_SCL         ,
    inout  wire                 QSFP112_SDA         ,
    output wire                 QSFP112_OE_n        ,
    output wire                 QSFP112_RST_n       ,
    output wire                 QSFP112_LPMODE      ,
    input  wire                 QSFP112_MODPRS_n    ,
    input  wire                 QSFP112_INT_n       
);
	...
	wire [15:0]getKeepSignal_wire;
	wire [3:0]trigger_transfer_val;

	assign QSFP112_SCL = 1'bz;
	assign QSFP112_SDA = 1'bz;
	assign QSFP112_LPMODE = 1'b0;
	// assign QSFP112_RST_n = 1'b1;
	assign QSFP112_OE_n = 1'b0;
	...
	// anlt anlt_inst (
	// 	.i_clk                        (ftile_eth_reconfig_clk),                        //   input,   width = 1,             i_clk.clk
	// 	.i_reset                      (ftile_eth_rst_n),                      //   input,   width = 1,           i_reset.reset
	// 	.anlt_link                    (ftile_eth_anlt_link),                    //   input,   width = 1,         anlt_link.anlt_link
	// 	.i_kr_reconfig_addr           (),           //   input,  width = 12, kr_reconfig_slave.address
	// 	.i_kr_reconfig_read           (),           //   input,   width = 1,                  .read
	// 	.i_kr_reconfig_write          (),          //   input,   width = 1,                  .write
	// 	.i_kr_reconfig_byte_en        (),        //   input,   width = 4,                  .byteenable
	// 	.i_kr_reconfig_writedata      (),      //   input,  width = 32,                  .writedata
	// 	.o_kr_reconfig_readdata       (),       //  output,  width = 32,                  .readdata
	// 	.o_kr_reconfig_readdata_valid (), //  output,   width = 1,                  .readdatavalid
	// 	.o_kr_reconfig_waitrequest    ()     //  output,   width = 1,                  .waitrequest
	// );
	...
	system_clk_and_ftile_ref_clk system_clk_and_ftile_ref_clk_inst (
		...
		.in_refclk_fgt_3           (qsfpdd0_refclk_fgt),        	//   input,  width = 1,                refclk_fgt.in_refclk_fgt_3
		.in_refclk_fht_0           (qsfpdd0_refclk_fht),         //   input,  width = 1,                refclk_fht.in_refclk_fht_0
	...
	reg   tx_lanes_stable_all_sync;
	reg   ninit_done_sync;

	always @ (posedge ftile_eth_clk_pll) 
	begin
		tx_lanes_stable_sync <= ftile_eth_tx_lanes_stable;
		tx_lanes_stable_all <= tx_lanes_stable_sync;
	end

	always @(posedge ftile_eth_reconfig_clk)
	begin
		ninit_done_sync   <= ninit_done;
		tx_lanes_stable_all_sync <= tx_lanes_stable_all;
	end

	always @ (posedge ftile_eth_reconfig_clk) begin
		if(ninit_done_sync)
	...
	assign ftile_eth_reconfig_reset = (reset_till_lanes_stable); 
	...
	ftile_eth_hip ftile_eth_hip_inst (
		...
		.i_clk_pll                       (1'b0)                        //   input,     width = 1,             i_clk_pll.clk
    ...
    mkBsvTop bsv_top(
		...
		.trigger_transfer_val(trigger_transfer_val),
		...
		.getKeepSignal(getKeepSignal_wire)
    );

	issp issp_inst (
		.source ({trigger_transfer_val, QSFP112_RST_n}), //  output,   width = 1, sources.source
		.probe  (getKeepSignal_wire)   //   input,  width = 16,  probes.probe
	);
