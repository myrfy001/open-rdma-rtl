...
typedef TDiv#(MAX_PKT_BYTE_NUM, DATA_BUS_BYTE_WIDTH) MAX_PKT_FRAME_NUM;
...
typedef 1 CMAC_AXIS_TUSER_WIDTH;
typedef Bit#(CMAC_AXIS_TUSER_WIDTH) CmacAxisTuser;
typedef Bit#(DATA_BUS_BYTE_WIDTH) CmacAxisTkeep;
typedef AxiStream#(DATA, CmacAxisTkeep, CmacAxisTuser) CmacAxiStream;

(* always_ready, always_enabled *)
interface XilinxCmacTxController;
    // CMAC AXI-Stream Bus
    (* prefix = "cmac_tx_axis" *)
    interface RawAxiStreamMaster#(DATA, CmacAxisTkeep, CmacAxisTuser) rawCmacAxiStreamOut;
...
module mkXilinxCmacTxController#(
    ...
    PipeOut#(CmacAxiStream) userAxiStreamIn,
    ...
    FIFOF#(CmacAxiStream) cmacAxiStreamOutQueue <- mkFIFOF;
    FIFOF#(CmacAxiStream) interAxiStreamBuf <- mkSizedBRAMFIFOF(valueOf(CMAC_INTER_BUF_DEPTH));
            ...
            for (Integer i = 0; i < valueOf(VIRTUAL_CHANNEL_NUM); i = i + 1) begin
                if (flowControlReqVec[i] matches tagged Valid .ctrlReq) begin
                    ...
                    if (ctrlReq == FLOW_CTRL_PASS) begin
                        ...
                        ctlTxPauseQuantaRegVec[i] <= maxBound;
                    end
                end
            end
    ...
    rule genFullPacketAxiStreamOut;
        ...
        cmacAxiStreamOutQueue.enq(axiStream);
        if (axiStream.axisLast == packetReady) begin
            packetReadyInfoBuf.deq;
        end
    endrule
    
    let rawCmacAxiStream <- mkPipeOutToRawAxiStreamMaster(toPipeOut(cmacAxiStreamOutQueue));
...
interface XilinxCmacRxController;
    // CMAC AXI-Stream Bus
    (* prefix = "cmac_rx_axis" *)
    interface RawAxiStreamSlave#(DATA, CmacAxisTkeep, CmacAxisTuser) rxRawAxiStreamIn;
...
typedef struct {
    ...
    CmacAxiStream axiStream;
} CmacAxiStreamWithTag deriving(Bits, FShow);
...
module mkXilinxCmacRxController#(
    Bool isEnableRsFec,
    Bool isEnableFlowControl,
    PipeIn#(CmacAxiStream) userAxiStreamOut,
    ...
    Wire#(Maybe#(CmacAxiStream)) rxAxiStreamInW <- mkBypassWire;
    ...
    FIFOF#(CmacAxiStreamWithTag) rxAxiStreamInterBuf <- mkSizedBRAMFIFOF(valueOf(CMAC_INTER_BUF_DEPTH));
    FIFOF#(CmacAxiStream) rxAxiStreamOutBuf <- mkFIFOF;
    ...
    rule enqRxAxiStreamInterBuf if (rxStateReg == RX_STATE_AXIS_ENABLE && isValid(rxAxiStreamInW));
        ...
        if (isThrowFaultFrame) begin
            isThrowFaultFrame <= !rxAxiStream.axisLast;
        end
        else begin
            // check if frame is corrupted or inter buffer is full
            if (rxAxiStreamInterBuf.notFull && rxAxiStream.axisUser==0) begin
                rxAxiStreamInterBuf.enq(
                    CmacAxiStreamWithTag {
    ...
    interface RawAxiStreamSlave rxRawAxiStreamIn;
        method Bool axisReady = rxAxiStreamInterBuf.notFull;
        method Action axisValid(
            Bool valid,
            DATA axisData, 
            CmacAxisTkeep axisKeep, 
            Bool axisLast, 
            CmacAxisTuser axisUser
        );
            if (valid) begin
                rxAxiStreamInW <= tagged Valid CmacAxiStream {
                    axisData: axisData,
                    axisKeep: axisKeep,
                    axisLast: axisLast,
                    axisUser: axisUser
...
interface RawXilinxCmacController;
    ...
    (* prefix = "user_tx_axis" *)
    interface RawAxiStreamSlave#(DATA, CmacAxisTkeep, CmacAxisTuser) rawUserAxiStreamTxIn;
    (* prefix = "user_tx_flowctrl" *)
    interface AxiRawBusSlave#(FlowControlReqVec) rawUserFlowCtrlReqVecTxIn;
    
    (* prefix = "user_rx_axis" *)
    interface RawAxiStreamMaster#(DATA, CmacAxisTkeep, CmacAxisTuser) rawUserAxiStreamRxOut;
    (* prefix = "user_rx_flowctrl" *)
    interface AxiRawBusMaster#(FlowControlReqVec) rawUserFlowCtrlReqVecRxOut;
...
module mkRawXilinxCmacController(
    ...
    FIFOF#(CmacAxiStream) userAxiStreamTxInBuf <- mkFIFOF(reset_by cmacTxReset);
    FIFOF#(CmacAxiStream) userAxiStreamRxOutBuf <- mkFIFOF(reset_by cmacRxReset);
    ...
    let cmacController <- mkXilinxCmacController(
        ...
        isEnableFlowControl,
        isTxWaitRxAligned,
        toPipeOut(userAxiStreamTxInBuf ),
        toPipeIn (userAxiStreamRxOutBuf),
        toPipeOut(userFlowCtrlReqVecTxInBuf),
        toPipeIn (userFlowCtrlReqVecRxOutBuf),
        cmacRxReset,
        cmacTxReset
    );

    let userAxiStreamTxIn <- mkPipeInToRawAxiStreamSlave(
        toPipeIn(userAxiStreamTxInBuf),
        reset_by cmacTxReset
    );
    let userAxiStreamRxOut <- mkPipeOutToRawAxiStreamMaster(
        toPipeOut(userAxiStreamRxOutBuf),
        reset_by cmacRxReset
    );
    let userFlowCtrlReqVecTxIn <- mkPipeInToRawBusSlave(
        toPipeIn(userFlowCtrlReqVecTxInBuf),
        reset_by cmacTxReset
    );
    let userFlowCtrlReqVecRxOut <- mkPipeOutToRawBusMaster(
        toPipeOut(userFlowCtrlReqVecRxOutBuf),
        reset_by cmacRxReset
    );
