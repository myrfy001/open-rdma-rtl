...
(* doc = "testcase" *)
module mkTestBiDirectionStreamShifter(Empty);
    ...
    FIFOF#(DataStream) checkerExpectedResultQ <- mkSizedFIFOF(10);
    FIFOF#(DataStream) dutInputQ <- mkSizedFIFOF(10);
    FIFOF#(DataStream) forwardToBackwardQ <- mkSizedFIFOF(10);

    let forwardShifter <- mkBiDirectionStreamShifter;
    let backwardShifter <- mkBiDirectionStreamShifter;
        ...
        if (dutInputQ.notEmpty) begin
            ...
            if (ds.isFirst) begin
                ...
                if (msb(offset) == 0) begin
                    ...
                    Length maxAvailableShiftNum = zeroExtend(ds.byteNum) + zeroExtend(ds.startByteIdx);
                    offsetAcceptable = maxAvailableShiftNum > zeroExtend(absOffset);
                end
                else begin
                    // this is a negative number, means shift left.
                    Length maxAvailableShiftNum =  fromInteger(valueOf(DATA_BUS_BYTE_WIDTH)) - zeroExtend(ds.byteNum) - zeroExtend(ds.startByteIdx);
                    offsetAcceptable = maxAvailableShiftNum > zeroExtend(absOffset);
                end
